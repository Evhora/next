...
import type { DreamProps } from "../domain/Dream";
import { DREAM_AREA_OF_LIFE_LABELS } from "../domain/DreamAreaOfLife";
import {
  DREAM_STATUS_LABELS,
  DreamStatus,
...
/**
 * Map a numeric DreamStatus to the camelCase translation key the i18n bundle
 * uses (`pages.dreams.status.inProgress`, etc). Centralised so the map lives
 * next to the only place that needs it.
 */
const statusTranslationKey = (status: DreamStatus): string => {
  switch (status) {
    case DreamStatus.IN_PROGRESS:
      return "inProgress";
    case DreamStatus.PAUSED:
      return "paused";
